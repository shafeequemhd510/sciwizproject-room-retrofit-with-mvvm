...
public class ApiResponse<T> {
    ...
    public ApiResponse<T> create(Response<T> response){
        ...
        if(response.isSuccessful()){
            ...
            if(body instanceof FilimsResponse){
                if(!CheckRecipeApiKey.isRecipeApiKeyValid((FilimsResponse)body)){
                    String errorMsg = "Api key is invalid or expired.";
                    return new ApiErrorResponse<>(errorMsg);
                }
                if(((FilimsResponse) body).getCount() == 0){
                    // query is exhausted
                    return new ApiErrorResponse<>(null);
                }
            }
